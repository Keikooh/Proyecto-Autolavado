<?php
    ...
    if (isset($_POST['tReportes'],$_POST['tEmpleados'])) {
        ...
        $pdf->Cell(0, 10, '- El total de ganancias obtenidas de la fecha '.($fechaInicio = ($_POST['txtFechaInicio'] == '') ? date('Y-m-d') : $_POST['txtFechaInicio']).' al '.($fechaFinal = ($_POST['txtFechaFinal'] == '') ? date('Y-m-d') : $_POST['txtFechaFinal']).' fueron de '.$accesoDatos->obtenerGananciasDia($fechaInicio, $fechaFinal), 0, 1);
